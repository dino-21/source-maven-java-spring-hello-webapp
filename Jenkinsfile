  ...
  stages {
    ...
    stage('Tag Container Image') {
      ...
      steps {
        ...
        sh "docker image tag myhello:v1 dino-2/myhello:v${BUILD_NUMBER}"
        sh "docker image tag myhello:v1 dino-2/myhello:latest"
      }
    }
    ...
    stage('Push Container Image') {
      ...
      steps {
        withDockerRegistry(credentialsId: 'docker-registry-credential', url: 'https://index.docker.io/v1/') {
          ...
          sh "docker image push dino-2/myhello:v${BUILD_NUMBER}"
          sh "docker image push dino-2/myhello:latest"
        }
      }
    }
    ...
    stage('Run Container') {
      ...
      steps {
        ...
        sh 'docker container run --detach --name myhello -p 80:8080 dino-2/myhello:latest'
      }
    }
  }
}
